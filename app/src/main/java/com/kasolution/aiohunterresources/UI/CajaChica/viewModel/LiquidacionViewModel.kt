...
import com.kasolution.aiohunterresources.domain.cajaChica.getIdSheetLiquidacionUseCase
...
class LiquidacionViewModel : ViewModel() {
    ...
    val getIdSheetLiquidacion= MutableLiveData<Result<file>>()
    ...
    var getIdSheetLiquidacionUseCase = getIdSheetLiquidacionUseCase()
    ...
    fun createLiquidacionSheet(urlid: urlId) {
        viewModelScope.launch {
            try {
                setLoading(true)
                val response = getIdSheetLiquidacionUseCase(urlid)
                if (response.isSuccess) {
                    response.getOrNull()?.let { registro ->
                        getIdSheetLiquidacion.postValue(Result.success(registro))
                    }
                } else {
                    response.exceptionOrNull()?.let { ex ->
                        getIdSheetLiquidacion.postValue(Result.failure(ex))
                    }
                }
            } catch (e: Exception) {
        ...
        viewModelScope.launch {
            try {
                ...
                if (response.isSuccess) {
                    ...
                    response.exceptionOrNull()?.let { ex ->
                        getLiquidacion.postValue(Result.failure(ex))
                    }
                }
            } catch (e: Exception) {
                exception.postValue(e.message)
